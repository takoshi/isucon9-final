	...
	*/
	...
	case 0:
		...
		for _, carnum := range carNumList {
			...
			reservationIDListUsedMap := make(map[int]bool)
			for _, seatReservation := range seatReservationList {
				...
				if _, exist := reservationIDListUsedMap[seatReservation.ReservationId]; exist {
					continue
				}
				reservationIDListUsedMap[seatReservation.ReservationId] = true
			...
			reservationByID := make(map[int]Reservation)
			for _, reservation := range reservationList {
				reservationByID[reservation.ReservationId] = reservation
			}
			...
			for _, seat := range seatList {
				...
				var reservationList []Reservation
				for _, seatReservation := range seatReservationList {
					reservationList = append(reservationList, reservationByID[seatReservation.ReservationId])
				}
				...
				for _, reservation := range reservationList {
					...
					arrivalStation, exist = getStation(reservation.Arrival)
					if !exist {
						tx.Rollback()
						panic(err)
					}
	...
	for _, reservation := range reservations {
		...
		// From
		reservedfromStation, exist := getStation(reservation.Departure)
		if !exist {
			tx.Rollback()
		...
		// To
		reservedtoStation, exist = getStation(reservation.Arrival)
		if !exist {
			tx.Rollback()
		...
		if secdup {
			dupReservationIdList = append(dupReservationIdList, reservation.ReservationId)
		}
	}
...
var (
	...
	cancelPaymentIdLock sync.Mutex
)

// キャンセルする支払いIDを追加する
func pushCancelPaymentId(paymentId string) {
	cancelPaymentIdLock.Lock()
	cancelPaymentIdList = append(cancelPaymentIdList, paymentId)
	cancelPaymentIdLock.Unlock()
}
...
func userReservationCancelHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "done":
		...
		// キャンセル処理をpushする
		pushCancelPaymentId(reservation.PaymentId)
